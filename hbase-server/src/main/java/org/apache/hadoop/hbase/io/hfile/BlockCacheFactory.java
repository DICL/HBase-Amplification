...
public final class BlockCacheFactory {
  ...
  public static BlockCache createBlockCache(Configuration conf) {
    ...
    if (useExternal) {
      ...
      return l2CacheInstance == null ?
          l1Cache :
          new InclusiveCombinedBlockCache(l1Cache, l2CacheInstance, null);
    } else {
      // otherwise use the bucket cache.
      RowCache rowCache = createRowCache(conf);
      BucketCache bucketCache = createBucketCache(rowCache, conf);
      rowCache.setBucketCache(bucketCache);
      ...
      return bucketCache == null ? l1Cache : new CombinedBlockCache(l1Cache, bucketCache, rowCache);
    }
  }
  ...
  private static BucketCache createBucketCache(RowCache rowCache, Configuration c) {
    ...
    try {
      ...
      bucketCache = new BucketCache(bucketCacheIOEngineName,
          ...
          ioErrorsTolerationDuration, rowCache, c);
  ...
  private static RowCache createRowCache (Configuration c) {
    RowCache rowCache = null;
    String rowCacheIOEngineName = c.get(BUCKET_CACHE_IOENGINE_KEY, null);
    if (rowCacheIOEngineName == null || rowCacheIOEngineName.length() <= 0) {
     return null;
    }
    final long rowCacheSize = MemorySizeUtil.getBucketCacheSize(c);
    try {
      rowCache = new RowCache(rowCacheIOEngineName, rowCacheSize);
    } catch (IOException ioex) {
      LOG.error("Can't instantiate row cache", ioex); throw new RuntimeException(ioex);
    }
    return rowCache;
  }
}
