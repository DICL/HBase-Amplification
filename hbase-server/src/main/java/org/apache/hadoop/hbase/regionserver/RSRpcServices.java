...
public class RSRpcServices implements HBaseRPCErrorHandler,
  ...
  private final LongAdder entire_Time = new LongAdder();
  private final LongAdder rowCache_getTime = new LongAdder();
  private final LongAdder bucketCache_getTime = new LongAdder();
  private final LongAdder InitTime_ = new LongAdder();
  private final LongAdder InitTime = new LongAdder();
  private final LongAdder SeekTime = new LongAdder();
  private final LongAdder SearchTime = new LongAdder();
  private static final long NANO_TIME = TimeUnit.MILLISECONDS.toNanos(1);
  private static double totalRequest = 0;
  private static double totalIO = 0;
  private static double totalAmp = 0;
  transient HashMap<String, Integer> keyMap = new HashMap<>();
  transient HashMap<String, Integer> rowMap = new HashMap<>();
  ...
  public static int BeingCached = 0;

  public static int NBeingCached = 0;
  ...
  //  RowCache rowCache = region.getBlockCache().getrowCache();
   // Cell cell = rowCache.getCell(row);
    
   // if (cell != null){
   //   results.add(cell);
   //   rowCache_getTime.add(System.nanoTime() - rowStart);
   // }
    ...
    //if (cell == null) {
      ...
      bucketCache_getTime.add(System.nanoTime() - bucketStart);
      entire_Time.add(System.nanoTime() - rowStart);
    //}
/*
    if (access_num % 100000 == 0) { // Time breakdown.
      long entireTime = entire_Time.sum() / NANO_TIME;
      long rowTime = rowCache_getTime.sum() / NANO_TIME;
      long bucketTime = bucketCache_getTime.sum() / NANO_TIME;
      long initTime = InitTime_.sum() / NANO_TIME;
      long searchTime = SearchTime.sum() / NANO_TIME;

      LOG.info("[DICL] Entire Time=" + entireTime + ", " +
		      "RowCache Get Time=" + rowTime + ", " +
		      "Vanilla Get Time=" + bucketTime + ", " +
		      "Init Time=" + initTime + ", " +
		      "Search Time=" + searchTime);

      double rowP = (double) rowTime / (double) entireTime;
      double bucketP = (double) bucketTime / (double) entireTime;
      double initP = (double) initTime / (double) bucketTime;
      double searchP = (double) searchTime / (double) bucketTime;
      LOG.info("[DICL] RowCache Get Time Proportion=" + Math.round(rowP *100)/100.0 + ", " +
		      "Vanilla Get Time Proportion=" + Math.round(bucketP * 100)/100.0 + ", " +
		      "Init Time Proportion=" + Math.round(initP * 100)/100.0 + ", " +
		      "Search Time Proportion=" + Math.round(searchP * 100)/100.0);
    }*/
    ...
    for (int idx = 0; idx < results.size(); idx++){
       ApplicationIO = results.get(idx).getValueLength();
       ioCal.add_AppIO(results.get(idx).getValueLength());
       int mod = ApplicationIO % 4096 != 0 ? 1: 0;
       int page = ApplicationIO / 4096;
       ioCal.add_AppPage(mod+page);
    }
 
    if (access_num % 1000 == 0) {
      LOG.info("[DICL] Request Count=" + access_num + ", " +
		       "Application IO=" + ioCal.get_AppIO()/(1024*1024) + " MB, " +
		       "Disk IO=" + ioCal.get_realDiskIO()/(1024*1024) + " MB, " +
		       "Read Amplification=" +
		       Math.round((double)ioCal.get_DiskIO() / (double)ioCal.get_AppIO() * 1000)/1000.0 + ", " +
      		       "IO Time=" + ioCal.get_ioTime() + ", " +
		       "Application Page=" + ioCal.get_AppPage() + ", " +
		       "Disk Page=" + ioCal.get_DiskPage() + ", " +
		       "RAF=" +
		       Math.round((double)ioCal.get_DiskPage() / (double)ioCal.get_AppPage() * 1000)/1000.0);
    }
