...
public class HFileBlock implements Cacheable {
  ...
  static class Header {
    ...
    static int ENTRY_NUM_INDEX = 24;
    static int CHECKSUM_TYPE_INDEX = 26;
    static int BYTES_PER_CHECKSUM_INDEX = 27;
    static int ON_DISK_DATA_SIZE_WITH_HEADER_INDEX = 31;
  }
  ...
  public HFileBlock(BlockType blockType, int onDiskSizeWithoutHeader,
      int uncompressedSizeWithoutHeader, long prevBlockOffset, short entryNum, ByteBuff buf, boolean fillHeader,
  ...
  static HFileBlock createFromBuff(ByteBuff buf, boolean usesHBaseChecksum, final long offset,
    ...
    final short entryNum = buf.getShort(Header.ENTRY_NUM_INDEX);
  ...
  void sanityCheck() throws IOException {
    ...
    sanityCheckAssertion(dup.getShort(), entryNum, "entryNum");
    if (this.fileContext.isUseHBaseChecksum()) {
  ...
  public String toString() {
    StringBuilder sb = new StringBuilder()
      ...
      .append(", entryNum=").append(entryNum)
  ...
  static class Writer implements ShipperListener {
     ...
     * DICL Modification
     */
    private byte[] onDiskAlign = HConstants.EMPTY_BYTE_ARRAY;
    
    private int OS_PAGE_SIZE = 4096;
    ...
    public void setEntryCount(short rowCount){
	    entryCount = rowCount;
    }
    ...
    private void putHeader(byte[] dest, int offset, int onDiskSize,
      ...
      offset = Bytes.putShort(dest, offset, entryCount);
    ...
    /** DICL Modification finishBlockAndWriteHeaderAndData version2 */
/*
    protected void finishBlockAndWriteHeaderAndData(DataOutputStream out)
      throws IOException {
      ensureBlockReady();

      long alignSize = (startOffset + onDiskBlockBytesWithHeader.size() + onDiskChecksum.length) % OS_PAGE_SIZE;
      int index = (int)alignSize;
      onDiskAlign = new byte[index];
...
*/
   ...
   /** getOnDiskSizeWithHeader() Version 2 */
/*
    int getOnDiskSizeWithHeader() {
      expectState(State.BLOCK_READY);
      return onDiskBlockBytesWithHeader.size() + onDiskChecksum.length + onDiskAlign.length;
    }
*/
  ...
  static class FSReaderImpl implements FSReader {
	...
	ioCal.add_Duration(System.nanoTime() - startSeek);
    ...
    protected HFileBlock readBlockDataInternal(FSDataInputStream is, long offset,
	  ...
	  ioCal.add_realDiskIO((long)hdrSize);
          headerBuf.rewind();
        }
	...
	ioCal.add_realDiskIO(onDiskSizeWithHeader-preReadHeaderSize);
  ...
  public boolean equals(Object comparison) {
    ...
    if (castedComparison.entryNum != this.entryNum) {
      return false;
    }
