...
public final class BucketAllocator {
  ...
  final class BucketSizeInfo {
    ...
    public synchronized void removeForrowCache(Bucket b) {
      bucketList.remove(b);
      freeBuckets.remove(b);
      completelyFreeBuckets.remove(b);
    }
  ...
  BucketAllocator(long availableSpace, int[] bucketSizes) throws BucketAllocatorException {
    ...
    // DICL Modification
    this.victimIndex = buckets.length - 1;
  ...
  public Bucket retVictim() {
    int vicIdx = victimIndex;
    this.victimIndex = this.victimIndex - 1;
    return buckets[vicIdx];
  }

  public BucketSizeInfo getBucketSizeInfo(int idx) {
    return bucketSizeInfos[idx];
  }
   ...
   * DICL Modification
   */
  public Bucket grabCompletelyFreeBucketForRowCache() {
    for (BucketSizeInfo bsi : bucketSizeInfos) {
      Bucket b = bsi.findAndRemoveCompletelyFreeBucket();
      if (b != null) return b;
    }
