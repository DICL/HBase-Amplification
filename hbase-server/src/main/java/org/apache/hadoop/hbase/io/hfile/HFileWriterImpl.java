...
public class HFileWriterImpl implements HFile.Writer {
  ...
  /** Total # of key/value entries per block, i.e. how many times add() was called per block. */
  protected short rowCount = 0;

  /** Minimum block size. */
  protected long blockMinSize = 8192;

  protected double blockSize = 0;

  protected int blockNum = 0;

  protected double rowNum = 0;

  /** Optimal # of key/value entries per block, i.e. optimal times add() was called per block. */
  protected int OptimalBlockRowCount = 128;
 ...
 /*
  protected void checkBlockBoundary() throws IOException {
    ...
    if (blockWriter.encodedBlockSizeWritten() >= encodedBlockSizeLimit
      ...
      //LOG.info("[DICL] encodedBlockSizeWritten: " + blockWriter.encodedBlockSizeWritten() + " encodedBlockSizeLimit: " + encodedBlockSizeLimit + "\n");
      //LOG.info("[DICL] blockSizeWritten: " + blockWriter.blockSizeWritten() + " hFileContext.getBlockSize: " + hFileContext.getBlocksize() + "\n");
      //blockSize = blockSize + blockWriter.blockSizeWritten();
      //blockNum++;
      finishBlock();
      //LOG.info("[DICL] Avg block size : " + blockSize / blockNum + "\n");
...
*/
  ...
  protected void checkBlockBoundary() throws IOException {
    // checkBlockBoundary version 2 modified by dicl

    if (blockWriter.encodedBlockSizeWritten() >= encodedBlockSizeLimit
	|| blockWriter.blockSizeWritten() >= hFileContext.getBlocksize() 
	|| blockWriter.blockSizeWritten() >= blockMinSize
	&& rowCount > OptimalBlockRowCount) {
      //LOG.info("[DICL] encodedBlockSizeWritten : " + blockWriter.encodedBlockSizeWritten() + " encodedBlockSizeLimit : " + encodedBlockSizeLimit + "\n");
      //LOG.info("[DICL] blockSizeWritten : " + blockWriter.blockSizeWritten() + " : hFileContext.getBlockSize : " + hFileContext.getBlocksize() + "\n");
      //LOG.info("[DICL] blockSizeWritten2 : " + blockWriter.blockSizeWritten() + " : blockMinSize : " + blockMinSize + "\n");
      //LOG.info("[DICL] rowCount: " + rowCount + " OptimalBlockRowCount: " + OptimalBlockRowCount + "\n");
      //blockSize = blockSize + blockWriter.blockSizeWritten();
      //rowNum = rowNum + rowCount;
      //blockNum++;
      //LOG.info("[DICL] blockNum : " + blockNum + " Total Block size : " + blockSize + "\n");
      //LOG.info("[DICL] Avg block size : " + blockSize / blockNum + "\n");
      //LOG.info("[DICL] Avg #row : " + rowNum / blockNum + "\n");
      finishBlock();
      writeInlineBlocks(false);
      newBlock();
    }
  ...
  protected void newBlock() throws IOException {
    ...
    // Initialize rowCount state.
    //LOG.info("[DICL] rowCount: " + rowCount + "\n");
    blockWriter.setEntryCount(rowCount);
    rowCount = 0; 
