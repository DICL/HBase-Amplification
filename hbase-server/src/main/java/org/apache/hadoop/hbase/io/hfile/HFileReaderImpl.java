...
import org.apache.hadoop.hbase.regionserver.IOCalculate;
/**
...
public abstract class HFileReaderImpl implements HFile.Reader, Configurable {
  ...
  public static int cache_access = 0;
  ...
  protected static class HFileScannerImpl implements HFileScanner {
    ...
    private static int seek_count = 0;
    private static ArrayList<Cell> ArrList = new ArrayList<Cell>();
    private static HashSet<Cell> hset = new HashSet<Cell>();
    ...
    //DICL Modification
    private static short cellIndex = 0;
    ...
    void updateCurrBlockRef(HFileBlock block) {
      ...
      this.curBlock = block;
      this.cellIndex = 0; // Initialize cellIndex because curBlock is changed
    }
     ...
     *         key)
     */
    protected int blockSeek(Cell key, HFileBlock block, boolean seekBefore) {
      int klen, vlen, tlen = 0;
      int lastKeyValueSize = -1;
      int offsetFromPos;
      Optional<short[]> opt = null;
      short[] bitMap = null;

      //DICL Modification
      
      if(block.getBlockType() == BlockType.DATA || block.getBlockType() == BlockType.ENCODED_DATA){
      	BlockCacheKey cacheKey = 
          new BlockCacheKey(reader.getName(), block.getOffset(), true, block.getBlockType());
	opt = Optional.ofNullable(getCacheBitMap(cacheKey));
      }
    ...
    // DICL Modification
    public short[] getCacheBitMap(BlockCacheKey cacheKey)  {
      short[] BitMap = null;
      BlockCache cache = reader.getcacheConf().getBlockCache().orElse(null);
      try{
      	ConcurrentHashMap HashMap = cache.getBitMap();
      	BitMap = (short[])HashMap.get(cacheKey);
      }catch (NullPointerException e){
       //LOG.error("BitMap have NullPointerException\n");
      }
      //TODO: How to deal with null value
      return BitMap;
    }

    public void setCacheMeta() {
      Optional<short[]> opt = null;
      short[] bitMap = null;
      if (curBlock.getBlockType() == BlockType.DATA || curBlock.getBlockType() == BlockType.ENCODED_DATA) {
	BlockCacheKey cacheKey = new BlockCacheKey(reader.getName(), curBlock.getOffset(), true, curBlock.getBlockType());
        opt = Optional.ofNullable(getCacheBitMap(cacheKey));
	if (opt.isPresent()) {
	  try{
	    bitMap = opt.get();
	    ...
	    if (cellIndex > 0) {
	      short[] resizeMap = new short[cellIndex+1];
	      System.arraycopy(bitMap, 0, resizeMap, 0, bitMap.length);
	      resizeMap[cellIndex]++;
	      BlockCache cache = reader.getcacheConf().getBlockCache().orElse(null);
	      try {
	         ConcurrentHashMap hashMap = cache.getBitMap();
	         hashMap.replace(cacheKey, resizeMap);
	...
	cellIndex++; // Increment cellIndex inside the block
    ...
    protected int loadBlockAndSeekToKey(HFileBlock seekToBlock, Cell nextIndexedKey, boolean rewind,
      ...
      int retVal = blockSeek(key, seekToBlock, seekBefore);
      return retVal;
    }
  ...
  public static int readblock_access = 0;

  public static long IO_size = 0;
  
  public static int cache_hit = 0;

  public static int cache_miss = 0;

  public static long IO_time = 0;

  public final LongAdder IO_Time = new LongAdder();

  private static final long NANO_TIME = TimeUnit.MILLISECONDS.toNanos(1);

  public static final String BUCKET_CACHE_SIZE = "hbase.bucketcache.size";

  private IOCalculate ioCal = new IOCalculate();
  ...
  public HFileBlock readBlock(long dataBlockOffset, long onDiskBlockSize,
    ...
    boolean isMem = this.fileInfo.shouldIncludeMemStoreTS();
    boolean isTag = this.hfileContext.isIncludesTags();
    boolean isDec = this.fileInfo.isDecodeMemstoreTS();

    BlockCacheKey cacheKey = new BlockCacheKey(name, dataBlockOffset,
      this.isPrimaryReplicaReader(), expectedBlockType, isTag, isMem, isDec);
      ...
      while (true) {
        ...
        if (cacheConf.shouldReadBlockFromCache(expectedBlockType)) {
          ...
          if (cachedBlock != null) {
            ...
            // Cache-hit. Return!
	    if (expectedBlockType == BlockType.DATA || expectedBlockType == BlockType.ENCODED_DATA) {
	      cache_hit = cache_hit + 1;
	      ioCal.add_cacheHit(1);
	    }
            return cachedBlock;
          }
	...
	long ioStart = System.nanoTime();
        ...
        /** IO Quantity DICL LOG */	
	if (hfileBlock.getBlockType() == BlockType.DATA || hfileBlock.getBlockType() == BlockType.ENCODED_DATA){
	  readblock_access = readblock_access + 1;

	  long diskleft = dataBlockOffset % 4096;
	  int diskmod = dataBlockOffset % 4096 != 0 ? 1 : 0;
	  int DiskIO = hfileBlock.getOnDiskSizeWithoutHeader() - (int)diskleft;
	  int mod = DiskIO % 4096 != 0 ? 1 : 0;
	  int page = DiskIO / 4096;
	  ioCal.add_DiskPage(mod + page + diskmod);
	  ioCal.add_DiskIO(hfileBlock.getOnDiskSizeWithoutHeader());
	  IO_time = IO_time + (System.nanoTime() - ioStart);
	  ioCal.add_ioTime(System.nanoTime() - ioStart);
	  ioCal.add_ioAccess(1);
	  IO_Time.add(System.nanoTime() - ioStart);
	  long ioTime = IO_Time.sum();
	}
